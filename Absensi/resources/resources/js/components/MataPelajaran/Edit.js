...
export default class Edit extends Component {
    ...
    componentDidMount() {
        axios.get('http://127.0.0.1:8000/api/mata_pelajaran/' + this.props.match.params.id).then(response => {
            this.setState({kode_mata_pelajaran:response.data.kode_mata_pelajaran});
            this.setState({nama_mata_pelajaran:response.data.nama_mata_pelajaran});
        });
    }
    ...
    onSubmit(e) {
        e.preventDefault();
        const mataPelajaranChange = {
            kode_mata_pelajaran : this.state.kode_mata_pelajaran,
            nama_mata_pelajaran : this.state.nama_mata_pelajaran,
        }

        axios.patch('http://127.0.0.1:8000/api/mata_pelajaran/' + this.props.match.params.id, mataPelajaranChange)
